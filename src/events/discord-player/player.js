    ...
    player.events.on('playerError', (queue, error) => {
        ...
        if (error.message.includes('Sign in to confirm your age')) {
            if (queue.tracks.data.length < 1) {
                ...
                finishPlaying(queue);
                return queue.metadata.channel.send({ content: `❌ ไม่สามารถเล่นวีดีโอที่มีการจำกัดอายุได้`, allowedMentions: { repliedUser: false } });
            }

            else {
                queue.node.skip();
                return queue.metadata.channel.send({ content: `❌ ข้ามวีดีโอที่มีการจำกัดอายุแล้ว`, allowedMentions: { repliedUser: false } });
            }
        } else {
            queue.node.stop();
